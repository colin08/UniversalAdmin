   ...
   $.fn.Flowdesign = function(options)
    {
        var _canvas = $(this);
        //右键步骤的步骤号
        _canvas.append('<input type="hidden" id="leipi_active_id" value="0"/><input type="hidden" id="leipi_copy_id" value="0"/><input type="hidden" id="leipi_node_id" value="0"/>');
    ...
    if(processData.data)
    {
        $.each(processData.data, function(i,row) 
        {
            var nodeDiv = document.createElement('div');
            var nodeId = "window" + row.project_flow_node_id, badge = 'badge-inverse',icon = 'icon-star';
			...
			var nodeTop=row.top/10,nodeLeft=row.left/10;
            $(nodeDiv).attr("id",nodeId)
            .attr("style","top:"+nodeTop+"rem; left:"+nodeLeft+"rem;")
            .attr("process_to",row.process_to)
            .attr("process_id",row.project_flow_node_id)
			.attr("data-nodeId",row.node_id)
			.attr("data-nodeTt",row.node_title)
			.attr("data-nodeFa",row.node_is_fator)
			.attr("data-piece",row.piece) 
			.attr("data-color",row.color)
			.attr("data-icon",row.icon)
			.attr("data-isend",row.is_end)
			.attr("data-status",row.status)
            .addClass("process-step btn btn-small")
            //.html('<span class="process-flag badge '+badge+'"><i class="'+icon+' icon-white"></i></span>&nbsp;' + row.node_title )
            .html('<span class="process-flag badge '+badge+'" style="display:none;"><i class="'+icon+' icon-white"></i></span>&nbsp;' + row.node_title )
            .mousedown(function(e){
              if( e.which == 3 ) { //右键绑定
			      if(row.is_end){
					  warmMsgFunc(2,'该节点已完成');
					  return false;
				  }
			      $("#pmJudge,#pmAttribute").show();
                  _canvas.find('#leipi_active_id').val(row.project_flow_node_id);
				  _canvas.find('#leipi_node_id').val(row.node_id);
                  contextmenu.bindings = defaults.processMenus;
				  if(row.status){
					  $("#pmAttribute span").html('关闭节点');
				  }
				  else{
					  $("#pmAttribute span").html('开启节点');
					  $("#pmJudge").hide();					  
				  }
            ...
            //索引变量
            lastProcessId = row.project_flow_node_id;
        });//each
    }
    ...
    $(".process-step").live('click',function(){
		...
		_canvas.find('#leipi_node_id').val($(this).attr("data-nodeid")),
    ...
    //使之可拖动
    jsPlumb.draggable(jsPlumb.getSelector(".process-stepxx"));
    initEndPoints();
        ...
        $('.process-step').each(function(i){
            ...
            $.each(toArr,function(j,targetId){
                ...
                if(targetId!='' && targetId!=0){
                    ...
                    $.each(processData.data, function(i,row) 
                    {
                        if(row.project_flow_node_id == sourceId)
                        {
                            is_source = true;
                        }else if(row.project_flow_node_id == targetId)
                        {
                            is_target = true;
                        }
...
//-----外部调用----------------------
    ...
    var Flowdesign = {
        ...
        getNodeId:function()
        {
          return _canvas.find("#leipi_node_id").val();
        },
