   ...
   $.fn.Flowdesign = function(options)
    ...
    if(processData.list)
    {
        $.each(processData.list, function(i,row) 
        {
            ...
            var nodeId = "window" + row.flow_node_id, badge = 'badge-inverse',icon = 'fa-star';
            ...
            var div=$(nodeDiv).attr("id",nodeId)
            .attr("style",row.style)
            .attr("process_to",row.process_to)
            .attr("process_id",row.flow_node_id)
            .addClass("process-step btn btn-default")
            .html('<span class="process-flag badge '+badge+'"><i class="fa '+icon+'"></i></span>&nbsp;' + row.flow_node_title )
            .mousedown(function(e){
              if( e.which == 3 ) { //右键绑定
                  _canvas.find('#leipi_active_id').val(row.flow_node_id);
                  contextmenu.bindings = defaults.processMenus
                  $(this).contextMenu('processMenu', contextmenu);
              }
            });
            $.each(row,function(k,v){
            	if(/style|process_to/gi.test(k)){
            		return true;
            	}
            	div.attr('data-'+k,v);
            });
            ...
            //索引变量
            lastProcessId = row.flow_node_id;
  ...
  if($(this).height()<maxTop){
    ...
    var _canvas_design = function(){
        ...
        $('.process-step').each(function(i){
            ...
            var toArr =typeof prcsto =='undefined'?[]: prcsto.split(",");
            var processData = defaults.processData;
            $.each(toArr,function(j,targetId){
                ...
                if(targetId!='' && targetId!=0){
                    ...
                    $.each(processData.list, function(i,row) 
                    {
                        if(row.flow_node_id == sourceId)
                        {
                            is_source = true;
                        }else if(row.flow_node_id == targetId)
                        {
                            is_target = true;
                        }
...
//                          var icon='';
//                          $($(this).find('i').attr('class').split(' ')).each(function(){
//                        	  if(this=='fa'){
//                        		  return true;
//                        	  }
//                        	  icon=this;
//                          });
                         ...
                         if(!aProcessData[pId])
                          {
                              //aProcessData[pId] = {"top":0,"left":0,icon:icon,color:color, "process_to":[]};
                        	 item.process_to=[];
                        	 aProcessData[pId] =item;
                          }
             ...
             return aProcessData;//JSON.stringify(aProcessData);
          }catch(e){
              return '';
          }
