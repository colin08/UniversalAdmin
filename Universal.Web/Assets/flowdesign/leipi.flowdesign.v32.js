...
(function($) {
   ...
   var defaults = {
      ...
      mtAfterDrop:function(params){
    	 var process_to=$('#window'+params.sourceId).attr('data-process_to');
 		...
 		old.push(targetId);
   	  	$('#window'+params.sourceId).attr('data-process_to',old.join(','));
      },
      //这是连接线路的绘画样式
      connectorPaintStyle : {
          lineWidth:3,
   ...
   $.fn.Flowdesign = function(options)
    ...
    if(processData.list)
    {
        $.each(processData.list, function(i,row) 
        {
            ...
            var nodeId = "window" + row.id, badge = 'badge-inverse',icon = 'fa-star';
            ...
            .html('<span class="process-flag badge '+badge+'"><i class="fa '+icon+'"></i></span>&nbsp;' + row.node_title )
  ...
  if($(this).height()<maxTop){
    ...
    jsPlumb.bind("click", function(c) {
      if(confirm("你确定取消连接吗?"))
      	var sourceId=c.source.attr('data-id');
      	var targetId=parseInt(c.target.attr('data-id'));
