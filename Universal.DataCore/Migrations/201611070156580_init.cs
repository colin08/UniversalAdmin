namespace Universal.DataCore.Migrations
{
    ...
    public partial class init : DbMigration
    {
        public override void Up()
        {
            ...
            string sp_GetParentFlows = @"
                        CREATE PROCEDURE [dbo].[sp_GetParentFlows] (@Id int)
                        AS
                        BEGIN
                        WITH Record AS(
                        SELECT
                            ID,NodeID,Title,PID,TOID,Depth,Priority,IsDefault,AddTime,FlowName,TopPID,LastUpdateTime
                        FROM
                            Flow(NOLOCK)
                            WHERE Id=@Id
                            UNION ALL
                            SELECT
                            a.ID ID,a.NodeID NodeID,a.Title Title,a.PID PID,a.TOID TOID,a.Depth Depth,a.Priority Priority,a.IsDefault IsDefault,a.AddTime AddTime,a.FlowName FlowName,a.TopPID TopPID,a.LastUpdateTime LastUpdateTime
                        FROM
                            Flow(NOLOCK) a JOIN Record b
                            ON a.Id=b.PId
                        )
                        ...
                        END";
            //获取父级节点数据
            Sql(sp_GetParentFlows);
            string sp_GetChildFlows = @"
                    CREATE PROCEDURE [dbo].[sp_GetChildFlows] (@Id int)
                    AS
                    BEGIN
                    WITH Record AS(
                    SELECT
                    ...
                    END";
            //获取所有子流程节点
            Sql(sp_GetChildFlows);

            string fn_GetChildFlowStr = @"
                CREATE FUNCTION [dbo].[fn_GetChildFlowStr] (@Id int) RETURNS varchar(1000) 
                AS
                    BEGIN
                ...
                SELECT @a=isnull(@a+',','')+ltrim(Id) FROM Record
                return SUBSTRING(@a, 2, len(@a))
                END";
            //获取所有流程子节点的ID，逗号分割
            Sql(fn_GetChildFlowStr);
