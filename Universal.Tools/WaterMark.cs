...
namespace Universal.Tools
{
    public class WaterMark
    {
        ...
        {
            if (!File.Exists(IOHelper.GetMapPath(imgPath)))
                return;
            byte[] _ImageBytes = File.ReadAllBytes(IOHelper.GetMapPath(imgPath));
            Image img = Image.FromStream(new System.IO.MemoryStream(_ImageBytes));
            filename = IOHelper.GetMapPath(filename);

            if (watermarkFilename.StartsWith("/") == false)
                watermarkFilename = "/" + watermarkFilename;
            watermarkFilename = IOHelper.GetMapPath(watermarkFilename);
